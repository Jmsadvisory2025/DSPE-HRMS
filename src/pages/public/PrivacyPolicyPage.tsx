...
const RICH_SUBHEADINGS = [

  'Google API Data and AI/ML Data Handling',
...
const SECTIONS = [
...

  {
    ...
    after: 'We only request the information necessary for user authentication. We do not access your Gmail, Google Drive, Google Calendar, or any other Google services unless you explicitly authorize additional permissions. When authorized, RECRUIT-OS may send emails on your behalf and access your Gmail signature settings to include it in outgoing communications, in accordance with the scopes you approve during sign-in.',
...

  {
    ...
    title: 'Google API Data and AI/ML Data Handling',

    content: 'RECRUIT-OS uses Google APIs to authenticate users and provide functionality that users explicitly authorize. Google user data obtained through Google APIs is handled in accordance with the Google API Services User Data Policy, including the Limited Use requirements.\n\nGoogle User Data\n\nRECRUIT-OS only accesses Google user data that is necessary for the functionality requested and authorized by the user.\n\nRECRUIT-OS does not obtain resumes or candidate documents from Gmail, Google Drive, Google Calendar, Google Photos, or other Google Workspace or Google Photos services.\n\nResumes and candidate documents used for resume parsing are uploaded directly to RECRUIT-OS by users from their local device.\n\nAI/ML Processing\n\nRECRUIT-OS uses OpenAI GPT-4 for resume parsing. A resume uploaded directly by a user from their local device may be transmitted to OpenAI solely for the purpose of providing the requested resume parsing functionality.\n\nGoogle user data obtained through Google APIs is not transmitted to OpenAI for resume parsing, AI processing, model training, or any other secondary purpose.\n\nRECRUIT-OS does not use Google user data, including raw, aggregated, anonymized, or derived Google user data, to create, train, or improve generalized or foundational artificial intelligence or machine learning models.\n\nRECRUIT-OS does not sell Google user data or use Google user data for advertising or other unrelated purposes.\n\nGoogle user data is not transferred to third parties for the purpose of training or improving generalized or foundational AI/ML models.\n\nLimited Use Compliance\n\nThe use of raw or derived user data received from Google Workspace APIs will adhere to the Google API Services User Data Policy, including the Limited Use requirements.\n\nRECRUIT-OS uses Google user data only to provide or improve the user-facing functionality requested by the user and does not use Google user data for prohibited purposes.',
...
<p className="text-sm leading-relaxed mt-3" style={{ color: theme.textSecondary }}>
          ...
          {SECTIONS.map(({ title, content, bullets, after, richText }, index) => (
<div key={index}>
<h2 className="text-xl font-bold mb-3" style={{ color: theme.textPrimary }}>
              ...
              {richText ? (
<div className="space-y-3 text-sm leading-relaxed" style={{ color: theme.textSecondary }}>

                  {content.split('\n\n').map((paragraph, paragraphIndex) => {

                    const isSubheading = RICH_SUBHEADINGS.includes(paragraph);

                    const isHighlighted =

                      paragraph.startsWith('The use of raw or derived user data') ||

                      paragraph.startsWith('RECRUIT-OS uses Google user data only');
 
                    if (isSubheading) {

                      return (
<h3 key={paragraphIndex} className="pt-2 text-sm font-bold" style={{ color: theme.textPrimary }}>
