...
import { Loader2, Briefcase, MapPin, IndianRupee, Clock, Building2, GraduationCap, Download, Users, FileText, AlertCircle } from 'lucide-react';
  ...
  return (
    <Dialog open={isOpen} onOpenChange={(open) => !open && onClose()}>
      <DialogContent showCloseButton={!submitting} className="sm:max-w-[600px] w-full max-h-[90vh] flex flex-col">
        ...
        <div className="py-4 space-y-2 flex-shrink-0">
           <label className="text-sm font-medium" style={{ color: theme.textSecondary }}>Job Position</label>
           <Select value={selectedJobId} onValueChange={(val) => { setSelectedJobId(val || ''); setSubmissionErrors(null); }} disabled={submitting || jobsLoading}>
        ...
        <div className="overflow-y-auto pr-1 pb-1" style={{ minHeight: '100px' }}>
          {!detailLoading && selectedJobDetail && (
            <div className="rounded-lg p-5 space-y-4 mt-1 shadow-sm" style={{ background: theme.surfaceHover, border: `1px solid ${theme.border}` }}>
              {/* Header / Title */}
          ...
          {submissionErrors && (
            <div className="mt-4 rounded-lg p-4 border shadow-sm" style={{ background: theme.destructiveSoft, borderColor: theme.destructive + '40' }}>
              <div className="flex items-center gap-2 mb-2">
                ...
                <h4 className="text-sm font-bold" style={{ color: theme.destructive }}>
                  {submissionErrors.non_field_errors ? 'Submission Error' : 'Missing Candidate Details'}
                </h4>
              </div>
              {!submissionErrors.non_field_errors && (
                <p className="text-xs mb-3 font-medium opacity-90" style={{ color: theme.destructive }}>
                  Please update the candidate profile and fill in the following required fields before submitting to this job:
                </p>
              )}
        ...
        <DialogFooter className="mt-4 pt-4 flex-shrink-0" style={{ borderTop: `1px solid ${theme.border}` }}>
