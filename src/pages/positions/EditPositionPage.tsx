  ...
  useEffect(() => {
    if (selectedJob && selectedJob.id === jobId) {
      setFormData({
        ...
        budget: selectedJob.budget ? parseFloat(selectedJob.budget) : 0,
        job_type: selectedJob.job_type?.toLowerCase() as any || 'permanent',
        job_mode: selectedJob.job_mode?.toLowerCase() as any || 'office',
        hiring_for: selectedJob.hiring_for?.toLowerCase() as any || 'client',
        client: selectedJob.client_name ? clients.find(c => c.company_name === selectedJob.client_name)?.id || null : null,
        status: selectedJob.status?.toLowerCase() as any || 'open',
        assigned_recruiter_ids: selectedJob.assigned_recruiters?.map(r => r.id) || [],
        target_closing_date: selectedJob.target_closing_date || '',
        notice_period_preference: selectedJob.notice_period_preference || '',
        skill_criteria: selectedJob.skill_criteria ?? 70,
      });
    }
  }, [selectedJob, jobId, clients]);
