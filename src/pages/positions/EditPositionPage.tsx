...
const EditPositionPage = () => {
  ...
  const { selectedJob, detailLoading } = useAppSelector(
  ...
  const toggleRecruiter = (userId: string) => {
    setFormData((prev) => {
      ...
      if (current.includes(userId)) {
        return {
          ...prev,
          assigned_recruiter_ids: current.filter((id) => id !== userId),
        };
      }
  ...
  const handleSubmit = () => {
    ...
    if (!formData.team_member_id)
      errors.team_member_id = ["Client POC / Team Member must be selected."];
    ...
    if (formData.client) {
      ...
      if (formData.team_member_id) {
        fd.append("team_members", JSON.stringify([formData.team_member_id]));
      }
    }
    ...
    if (
      formData.assigned_recruiter_ids &&
      formData.assigned_recruiter_ids.length > 0
    ) {
      formData.assigned_recruiter_ids.forEach((id) =>
        fd.append("assigned_recruiter_ids", id),
      );
    }
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
            ...
            <div className="space-y-2">
              ...
              {selectedClientDetail && (
                <div
                  className="mt-2 p-3 rounded-md text-sm border"
                  style={{
                    background: theme.surface,
                    borderColor: theme.border,
                  }}
                >
                  <p
                    ...
                    style={{ color: theme.textPrimary }}
                  >
                    {selectedClientDetail.company_name}
                  </p>
                  <p
                  ...
                  >
                    <span>
                      {selectedClientDetail.industry || "No Industry"}
                    </span>{" "}
                    &bull;
                    <span>
                      {selectedClientDetail.city || "No City"},{" "}
                      {selectedClientDetail.country || ""}
                    </span>
                  </p>
                  <p
                    ...
                    style={{ color: theme.textMuted }}
                  >
                    Contact: {selectedClientDetail.contact}
                  </p>
                </div>
              )}
            </div>
            ...
            <div className="space-y-2">
              <label
                ...
                style={{ color: theme.textSecondary }}
              >
                Client POC / Team Member *
              </label>
              <SearchableDropdown
                options={clientTeamMembers.map((tm: any) => ({
                  value: tm.id,
                  label: tm.name,
                  description: tm.role || tm.designation || undefined,
                }))}
                value={formData.team_member_id || ""}
                onChange={(val) => {
                  setFormData({ ...formData, team_member_id: val });
                }}
                disabled={!formData.client || isFetchingClient}
                loading={isFetchingClient}
                placeholder={!formData.client ? "Select client first" : isFetchingClient ? "Loading POCs..." : "Search and select POC..."}
              />
              {formErrors.team_member_id && (
                <p className="text-xs" style={{ color: theme.destructive }}>
                  {formErrors.team_member_id[0]}
                </p>
              )}
              ...
              {formData.team_member_id && (
                ...
                >
                  {(() => {
                    const poc = clientTeamMembers.find(
                      (t) => t.id === formData.team_member_id,
                    );
                    if (!poc) return null;
                    return (
                      <>
            ...
            <div className="space-y-2">
              ...
              <Input
                ...
                style={{
                  background: theme.background,
                  borderColor: formErrors.min_experience
                    ? theme.destructive
                    : theme.border,
                  color: theme.textPrimary,
                }}
            ...
            <div className="space-y-2">
              ...
              <Input
                ...
                style={{
                  background: theme.background,
                  borderColor: formErrors.max_experience
                    ? theme.destructive
                    : theme.border,
                  color: theme.textPrimary,
                }}
          ...
          <div className="space-y-2">
              ...
              <Input
                ...
                onChange={(e) => {
                  if (e.target.files && e.target.files[0]) {
                    setFormData({
                      ...formData,
                      description_file: e.target.files[0],
                    });
                  }
                }}
            ...
            {selectedJob?.description_file && !formData.description_file && (
              <p className="text-xs" style={{ color: theme.textSecondary }}>
                Current file:{" "}
                <a
                  href={selectedJob.description_file as string}
                  target="_blank"
                  rel="noreferrer"
